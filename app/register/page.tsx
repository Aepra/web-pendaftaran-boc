...
export default function RegisterPage() {
  ...
  const [d, sd] = useState<RegistrationFormData>({
    teamName: "", leaderName: "", email: "", whatsapp: "",
    institution: "", category: "", paymentMethod: "",
    memberCount: 1, memberNames: "", notes: "",
  });
  const [st, sst] = useState<"idle" | "loading" | "success" | "error">("idle");
  const [em, sem] = useState("");
  const [r, sr] = useState<RegistrationResponse | null>(null);

  const pv = useMemo(
    () => (!d.category || !d.paymentMethod ? null : getPaymentPreview(d.category, d.paymentMethod)),
    [d.category, d.paymentMethod]
  );
  ...
  const sb = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (dt.status === "success" && dt.data) {
        ...
        sem(dt.message || "Gagal menyimpan data.");
        sst("error");
      }
    } catch {
      sem("Terjadi kesalahan jaringan.");
      sst("error");
    }
  };
          ...
          <div className="max-w-md mx-auto">
            <div className="p-8 rounded-2xl border border-amber-500/20 bg-zinc-900/50 shadow-[0_0_40px_rgba(245,158,11,0.08)]">
              <div className="text-center">
                <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 mb-6">
                  <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <h3 className="text-2xl font-extrabold text-amber-400 mb-2">Case Successfully Registered</h3>
                <p className="text-zinc-400 mb-8">Berkas investigasi Anda telah tercatat.</p>
              </div>
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="rounded-xl p-4 border border-zinc-800 bg-zinc-950/50">
                  <span className="text-xs text-amber-400/70 uppercase tracking-wider">Registration ID</span>
                  <p className="text-lg font-bold text-amber-300 mt-1">{r.data.registration_id}</p>
                </div>
                <div className="rounded-xl p-4 border border-zinc-800 bg-zinc-950/50">
                  <span className="text-xs text-amber-400/70 uppercase tracking-wider">Order ID</span>
                  <p className="text-lg font-bold text-amber-300 mt-1">{r.data.order_id}</p>
                </div>
              </div>
              <div className="rounded-xl p-4 border border-zinc-800 bg-zinc-950/50 mb-8">
                <h4 className="text-sm font-semibold text-amber-400 mb-3">Case Cost Summary</h4>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-zinc-400">Biaya Pendaftaran</span>
                    <span className="text-zinc-200">{fmt(r.data.base_amount)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-zinc-400">Biaya Admin</span>
                    <span className="text-zinc-200">{fmt(r.data.admin_fee)}</span>
                  </div>
                  <div className="border-t border-zinc-700 pt-2 flex justify-between">
                    <span className="font-semibold text-zinc-300">Total</span>
                    <span className="font-bold text-amber-400 text-base">{fmt(r.data.total_amount)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 rounded-2xl border border-zinc-800/60 bg-zinc-900/40">
                ...
                <ul className="space-y-3 text-sm">
                  <li className="flex gap-2">
                    ...
                    <span className="text-zinc-300">Matematika, Logika, Pengetahuan Umum, Pemecahan Masalah, Campuran</span>
                  </li>
                  <li className="flex gap-2">
              ...
              {pv && (
                <div className="mb-5 p-4 rounded-xl border border-amber-500/15 bg-amber-500/5">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-zinc-400">Biaya Pendaftaran</span>
                      <span className="text-zinc-200">{fmt(pv.baseAmount)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zinc-400">Biaya Admin</span>
                      <span className="text-zinc-200">{fmt(pv.adminFee)}</span>
                    </div>
                    <div className="border-t border-zinc-700 pt-2 flex justify-between">
                      <span className="font-semibold text-zinc-300">Total</span>
                      <span className="font-bold text-amber-400 text-base">{fmt(pv.totalAmount)}</span>
                    </div>
                  </div>
                </div>
              )}
